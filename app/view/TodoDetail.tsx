...
export function useTodoEdit(todo: Todo): any {
  ...
  const  [editableTodo, dispatch] = useReducer((state: EditableTodo, action: EditTodoActions) => {
    switch(action.type) {
      case "EditTitle":
    ...
    <div className={className`wrapper`}>
      <ul>
        ...
        { isEdittingTodo ? 
          ...
          : <li onClick={() => setEdittingTodo(true)} className={className`title`}>{todo.title}</li>
        }
      </ul>
    </div>
  )
